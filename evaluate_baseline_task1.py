...
def enhance_sound(predictors, model, device, length, overlap):
    ...
    for i in range(len(starts)):
        ...
        if 'audiovisual' in args.architecture:
            ...
            predicted_x = model(cut_x, images, device)
        else:
...
def main(args):
    ...
    image_predictors = predictors[1]
    predictors = np.array(predictors[0])
    ...
    dataset_ = CustomAudioVisualDataset((predictors, image_predictors), target, args.path_images, args.path_csv_images, transform)
    ...
    if args.architecture == 'fasnet':
        ...
        model = AMMB(16, 512, 2, 2, 4, 512, 128, 512, 0.1, 'cuda')
